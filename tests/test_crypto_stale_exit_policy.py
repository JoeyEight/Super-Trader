...
class CryptoStaleExitPolicyTests(unittest.TestCase):
    def test_entry_calibration_gate_blocks_low_trigger_reliability(self) -> None:
        pt_trader = _load_pt_trader_module()
        out = pt_trader.CryptoAPITrading._evaluate_entry_calibration_gate(
            calib_prob=0.86,
            calib_samples=80,
    ...
    def test_entry_calibration_gate_passes_when_confidence_and_trigger_quality_are_strong(self) -> None:
        pt_trader = _load_pt_trader_module()
        out = pt_trader.CryptoAPITrading._evaluate_entry_calibration_gate(
            calib_prob=0.88,
            calib_samples=120,
            trigger_reliability=0.79,
            min_calib_prob=0.55,
            min_calib_samples=10,
            min_trigger_reliability=0.55,
            min_entry_score=0.60,
    ...
    def test_manage_trades_holds_stale_loser_without_hard_reverse_signal(self) -> None:
        pt_trader = _load_pt_trader_module()
        bot = object.__new__(pt_trader.CryptoAPITrading)
        ...
        bot._dca_buy_ts = {}
        bot._dca_last_sell_ts = {}
        bot._last_entry_ts = {"BTC": now_ts - 14_400.0}
        bot._last_exit_ts = {}
        bot.entry_cooldown_seconds = 60.0
        bot._last_account_value_history_write_ts = 0.0
        bot._rate_limited_log_ts = {}
        ...
        bot.dca_levels_triggered = {}
        bot._pnl_ledger = {
            "open_positions": {"BTC": {"usd_cost": 100.0, "qty": 1.0, "opened_ts": now_ts - 14_400.0}},
            "pending_orders": {},
        }
        bot._last_good_account_snapshot = {
            "total_account_value": None,
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            pt_trader.read_settings_file = lambda path, module_name=None: {
                ...
                "crypto_stale_min_notional_usd": 1.0,
                "crypto_stale_min_hold_seconds": 3600,
                "crypto_stale_force_short_count": 4,
                "crypto_stale_require_hard_reverse_on_loss": True,
                "crypto_stale_force_loss_pct": -4.0,
            }
            pt_trader.time.sleep = lambda *_args, **_kwargs: None
        ...
        stale_events = latest.get("stale_exit_events", []) if isinstance(latest.get("stale_exit_events", []), list) else []
        reasons = [str((row or {}).get("reason", "") or "") for row in stale_events if isinstance(row, dict)]
        self.assertIn("stale_exit_loss_reverse_guard", reasons)
